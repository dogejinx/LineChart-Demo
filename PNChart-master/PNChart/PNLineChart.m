...
- (void)setYLabels:(NSArray *)yLabels withHeight:(CGFloat)height {
    ...
    if (_showLabel) {
        ...
        for (int index = 0; index < yLabels.count; index++) {
            ...
            float yLabelValue = [yLabels[index] floatValue];
            labelText = [format stringFromNumber:[NSNumber numberWithFloat:yLabelValue]];
...
- (void)drawAuxiliaryLine:(CGPoint)point line:(NSInteger)line index:(NSInteger)index {
    ...
    {
        PNLineChartDataModel *data = self.chartModelArray[index];
        NSString *str = data.showInLabelString;
...
- (void)setupYFixedValue:(NSArray *)array {
    float minValue = 0;
    float maxValue = 0;
    for (NSInteger i=0; i< array.count; i++) {
        float temp = [array[i] floatValue];
        if (temp < minValue) {
            minValue = temp;
        }
        if (temp > maxValue) {
    ...
    _yFixedValueMin = minValue;
    _yFixedValueMax = maxValue;
}
